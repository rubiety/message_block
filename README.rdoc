== Message Block
Implements the common pattern by which a list of messages are shown at the top, 
often a combination of flash messages (and potentially different types of them)
and ActiveRecord validation issues on one or more models.  
...
Imagine you have a form for entering a user and a comment:
  ...
  <%= message_block :on => [:user, :comment], :id => "messages" %>
...
The output with this and the user and comment objects failing validation is:
